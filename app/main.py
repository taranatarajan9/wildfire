...
def return_formatted(labels, confidences):
    output = ""
    for i, elem in enumerate(zip(labels, confidences)):
        label, confidence = elem
...
def results(filename): 
    ...
    if len(class_ids) > 0:
        ...
        formatted = return_formatted(labellist, conflist)
        note = "Note: The percentages given display the AI's level of confidence in each prediction."
        return render_template('results.html', confidences=format_confidences, labels=labels, 
            old_filename = filename, 
            filename=new_filename, formatted = formatted, note = note) 
    else:
        found = False
        # replace 'Objects' with whatever you are trying to detect
        return render_template('results.html', labels='No Objects', old_filename=filename, filename=filename, formatted="No signs of wildfire detected in the image. See below if you believe the prediction was incorrect.") 
